...
\subsubsection{Graphical User Interface (gui.py)}

	A graphical user interface was developed using Python's Tkinter library to provide a more user-friendly interaction with the compiler. The GUI is organized into four main tabs:
	
	\begin{itemize}
		\item \textbf{Output Tab}: Displays the complete compilation output, including lexical analysis results, parsing status, SDT verification, symbol table, and the AST in text format.
		\item \textbf{Symbol Table Tab}: Shows the symbol table in a structured table format with columns for variable name, type, and value.
		\item \textbf{AST (Text) Tab}: Presents the Abstract Syntax Tree in a hierarchical text format.
		\item \textbf{AST (Image) Tab}: Displays the graphical AST generated by Graphviz as a PNG image, with scroll support for large trees.
	\end{itemize}
	
	The GUI includes two main buttons:
	\begin{itemize}
		\item \textbf{Compile}: Triggers the lexical, syntactic, and semantic analysis on the code entered in the editor.
		\item \textbf{Clear All}: Clears the code editor and all result tabs.
	\end{itemize}
	
	A dependency checker module (\texttt{deps\_checker.py}) was implemented to verify at startup that all required libraries are installed:
	\begin{itemize}
		\item \textbf{Tkinter}: Mandatory for the GUI to function.
		\item \textbf{Pillow (PIL)}: Optional, required for displaying AST images.
		\item \textbf{Graphviz}: Optional, required for generating AST PNG images.
	\end{itemize}
...
\subsection{GUI Execution Results}

The graphical user interface was tested with the same input scenarios described in Section 3.2. Figures~\ref{fig:gui_test1} through~\ref{fig:gui_errors} show the GUI output for each test case. \footnote{ Only executable.py works with local files.}

\subsubsection{Test Case 1 - GUI Output}

Figure~\ref{fig:gui_test1} shows the GUI after compiling \texttt{float y = (2 + 3) * 4;}. The Output tab displays the compilation log, the Symbol Table tab shows the variable \texttt{y} with type \texttt{float} and value \texttt{20}, and the AST (Image) tab displays the graphical representation of the syntax tree.

\begin{figure}[h]
	\centering
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test1_output.png}
		\caption*{Output tab}
		\label{fig:gui_test1_output}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test1_symbols.png}
		\caption*{Symbol table tab}
		\label{fig:gui_test1_symbols}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test1_ast.png}
		\caption*{AST image tab}
		\label{fig:gui_test1_ast}
	\end{minipage}
	\caption{GUI results for Test Case 1: valid declaration and simple expression}
	\label{fig:gui_test1}
\end{figure}

\subsubsection{Test Case 2 - GUI Output}

Figure~\ref{fig:gui_test2} shows the GUI after compiling the nested expression \texttt{float y = ((2 + 3) * 4) + (10 * (5 + 1));}. The symbol table correctly shows the value 80, and the AST image displays the complete hierarchical structure with addition as the root node.

\begin{figure}[h]
	\centering
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test2_output.png}
		\caption*{Output tab}
		\label{fig:gui_test2_output}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test2_symbols.png}
		\caption*{Symbol table}
		\label{fig:gui_test2_symbols}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.32\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test2_ast.png}
		\caption*{AST image}
		\label{fig:gui_test2_ast}
	\end{minipage}
	\caption{GUI results for Test Case 2: Valid declaration with nested expressions}
	\label{fig:gui_test2}
\end{figure}

\subsubsection{Error Handling in GUI}

Figure~\ref{fig:gui_errors} demonstrate how the GUI presents syntax and semantic errors.

\begin{figure}[h]
	\centering
	\begin{minipage}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test3_error.png}
		\caption*{Syntax error: missing parenthesis and semicolon}
		\label{fig:gui_error_syntax}
	\end{minipage}
	\hfill
	\begin{minipage}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figs/gui_test5_error.png}
		\caption*{Semantic error: type mismatch (int = float)}
		\label{fig:gui_error_semantic}
	\end{minipage}
	\caption{GUI error display for syntax and semantic errors}
	\label{fig:gui_errors}
\end{figure}

\subsection{Dependency Checker}

Figure~\ref{fig:deps_checker} shows the dependency checker warnings that appear when optional dependencies (Pillow or Graphviz) are missing. The program continues to function, but with limited capabilities (no AST images).

\begin{figure}[h]
	\centering
	\includegraphics[width=0.37\textwidth]{figs/deps_warning.png}
	\caption{Dependency checker warning for missing optional dependencies}
	\label{fig:deps_checker}
\end{figure}
...
\section{Conclusions}
	...
	Additionally, a graphical user interface was implemented to improve the usability of the compiler. The GUI provides real-time feedback through four organized tabs, allowing users to view compilation outputs, symbol tables, and ASTs in both text and graphical formats. A dependency checker ensures that the system behaves gracefully when optional libraries (Pillow, Graphviz) are not installed, displaying clear warnings instead of crashing. This makes the tool accessible to users regardless of their development environment configuration.
